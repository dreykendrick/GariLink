...
let ConsoleSmsProvider = ConsoleSmsProvider_1 = class ConsoleSmsProvider {
    constructor() {
        ...
        if (process.env.NODE_ENV === 'production') {
            throw new Error('ConsoleSmsProvider cannot be used in production. Configure a real SMS provider.');
        }
    }
    async sendOtp(phoneNumber, code, purpose) {
        this.logger.log(`[DEV SMS] OTP to ${phoneNumber} for ${purpose}: ${code}`);
    }
    async sendWelcomeSms(phoneNumber, name) {
        this.logger.log(`[DEV SMS] Welcome to GariLink, ${name}! Sent to ${phoneNumber}`);
    }
};
